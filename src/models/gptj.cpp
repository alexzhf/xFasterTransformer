...
GPTJ<WeiT>::GPTJ(const std::string &modelPath)
    : CommonDecoder<Attention<WeiT, GPTJRotaryEmbedding, LayerNorm>, GPTJMLP<WeiT>, float, true>(modelPath, "gptj") {
    // Context
...
template <typename WeiT>
void GPTJ<WeiT>::setEmbeddingWeights(const std::string &modelPath) {
...
template <typename WeiT>
void GPTJ<WeiT>::setFinalLnWeight(const std::string &modelPath) {
    int hiddenSize = embedding->getHiddenSize();
...
template <typename WeiT>
void GPTJ<WeiT>::prepareAttnMask(int *ids, int step) {
    DecoderContext *ctx = this->getContext();
...
template <typename WeiT>
void GPTJ<WeiT>::embeddingForward(int *ids, float *output, int batchSize, int seqLen) {
    embedding->forward(ids, output, batchSize, seqLen);
}

template <typename WeiT>
void GPTJ<WeiT>::lastLayerNormForward(float *input, float *output, int rows) {
    finalLN.forward(input, output, rows);
}

template class GPTJ<float>;
template class GPTJ<float16_t>;
template class GPTJ<bfloat16_t>;
template class GPTJ<int8_t>;
template class GPTJ<w8a8_t>;
template class GPTJ<uint4x2_t>;
template class GPTJ<nf4x2_t>;
